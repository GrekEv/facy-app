# 🚀 Полная инструкция по развертыванию OnlyFace.art

## 📋 Содержание

1. [Подготовка сервера](#1-подготовка-сервера)
2. [Настройка базы данных PostgreSQL](#2-настройка-базы-данных-postgresql)
3. [Развертывание приложения](#3-развертывание-приложения)
4. [Настройка веб-приложения](#4-настройка-веб-приложения)
5. [Настройка Telegram Bot](#5-настройка-telegram-bot)
6. [Проверка работы](#6-проверка-работы)
7. [Мониторинг и обслуживание](#7-мониторинг-и-обслуживание)

---

## 1. Подготовка сервера

### 1.1. Создание ВМ в Яндекс.Облаке

1. Откройте https://cloud.yandex.ru
2. **Compute Cloud** → **Виртуальные машины** → **Создать ВМ**
3. Настройки:
   - **Имя:** `onlyface-server`
   - **Зона:** `ru-central1-a` (Москва)
   - **Платформа:** Intel Ice Lake
   - **vCPU:** 2
   - **RAM:** 4 GB
   - **Диск:** 20 GB SSD
   - **Образ:** Ubuntu 22.04 LTS
   - **Публичный IP:** ✅ Включить
   - **Пользователь:** `ubuntu`
   - **SSH ключ:** Добавьте ваш публичный ключ
4. Нажмите **"Создать"**
5. Скопируйте **публичный IP адрес** (например: `158.160.96.182`)

### 1.2. Настройка групп безопасности

1. В консоли Яндекс.Облака: **VPC** → **Группы безопасности**
2. Выберите группу безопасности вашей ВМ
3. Добавьте правила:

**Входящий трафик:**
- **Порт:** `22` (SSH)
- **Протокол:** TCP
- **Источник:** `0.0.0.0/0` (или ваш IP для безопасности)

- **Порт:** `80` (HTTP)
- **Протокол:** TCP
- **Источник:** `0.0.0.0/0`

- **Порт:** `443` (HTTPS)
- **Протокол:** TCP
- **Источник:** `0.0.0.0/0`

- **Порт:** `8000` (API)
- **Протокол:** TCP
- **Источник:** `0.0.0.0/0`

**Исходящий трафик:**
- Разрешить весь (по умолчанию)

### 1.3. Подключение к серверу

```bash
ssh ubuntu@158.160.96.182
```

---

## 2. Настройка базы данных PostgreSQL

### 2.1. Создание кластера PostgreSQL в Яндекс.Облаке

1. В консоли Яндекс.Облака: **Managed Databases** → **PostgreSQL** → **Создать кластер**

2. **Основные настройки:**
   - **Имя кластера:** `onlyface-postgresql`
   - **Окружение:** `Production`
   - **Версия PostgreSQL:** `15`

3. **Вычислительные ресурсы:**
   - **Класс хоста:** `s2.micro` (2 vCPU, 8 GB RAM) - от 500₽/мес
   - **Размер диска:** `20 GB SSD`

4. **База данных:**
   - **Имя БД:** `onlyface_db`
   - **Имя пользователя:** `onlyface_user`
   - **Пароль:** Сгенерируйте надежный пароль (сохраните!)

5. **Сеть:**
   - **Сеть:** Выберите сеть вашей ВМ
   - **Подсеть:** Выберите подсеть
   - **Публичный доступ:** ✅ **Включить**

6. **Группы безопасности:**
   - Добавьте правило для порта `6432` (PostgreSQL)
   - **Источник:** IP вашей ВМ `158.160.96.182` или подсеть

7. Нажмите **"Создать"** и дождитесь создания (5-10 минут)

### 2.2. Получение данных подключения

1. Откройте созданный кластер
2. Вкладка **"Подключение"**
3. Скопируйте:
   - **Хост:** `c-xxxxx.rw.mdb.yandexcloud.net`
   - **Порт:** `6432`
   - **Имя БД:** `onlyface_db`
   - **Пользователь:** `onlyface_user`
   - **Пароль:** (ваш пароль)

---

## 3. Развертывание приложения

### 3.1. Установка Docker и Docker Compose

**На сервере выполните:**

```bash
# Обновление системы
sudo apt update && sudo apt upgrade -y

# Установка Docker
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh
sudo usermod -aG docker ubuntu
rm get-docker.sh

# Установка Docker Compose
sudo curl -L "https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
sudo chmod +x /usr/local/bin/docker-compose

# Выход и повторный вход для применения изменений
exit
```

Подключитесь снова:
```bash
ssh ubuntu@158.160.96.182
```

### 3.2. Клонирование репозитория

```bash
# Установка Git
sudo apt install git -y

# Клонирование репозитория
git clone https://github.com/GrekEv/facy-app.git
cd facy-app

# Или если репозиторий приватный, используйте SSH ключ
```

### 3.3. Настройка переменных окружения

```bash
cd ~/facy-app
nano .env
```

**Добавьте следующие переменные:**

```env
# ============================================
# ОБЯЗАТЕЛЬНЫЕ ПЕРЕМЕННЫЕ
# ============================================

# Токен бота от BotFather
BOT_TOKEN=8254778202:AAH-1RebJBOKpr5fKorcIcFHqKAihbCBQ_o

# URL вашего Web App
WEBAPP_URL=https://onlyface.art

# Режим работы
ENVIRONMENT=production

# База данных PostgreSQL
DATABASE_URL=postgresql+asyncpg://onlyface_user:ВАШ_ПАРОЛЬ@c-xxxxx.rw.mdb.yandexcloud.net:6432/onlyface_db?ssl=require

# Сервер
HOST=0.0.0.0
PORT=8000

# ============================================
# API КЛЮЧИ ДЛЯ ГЕНЕРАЦИИ ИЗОБРАЖЕНИЙ
# ============================================

# Replicate (рекомендуется для изображений)
REPLICATE_API_KEY=your_replicate_api_key_here
REPLICATE_API_URL=https://api.replicate.com/v1

# ============================================
# API КЛЮЧИ ДЛЯ ГЕНЕРАЦИИ ВИДЕО
# ============================================

# OpenAI Sora (для генерации видео)
OPENAI_API_KEY=your_openai_api_key_here
SORA_MODEL=sora-1.0-pro

# ============================================
# ВЫБОР ПРОВАЙДЕРОВ
# ============================================

# Для генерации изображений
IMAGE_GENERATION_PROVIDER=replicate

# Для генерации видео
VIDEO_GENERATION_PROVIDER=sora
```

**Сохраните:** `Ctrl+O`, `Enter`, `Ctrl+X`

**Важно:** Замените:
- `ВАШ_ПАРОЛЬ` на пароль от PostgreSQL
- `c-xxxxx.rw.mdb.yandexcloud.net` на ваш хост PostgreSQL

### 3.4. Исправление всех проблем в коде

**Выполните на сервере:**

```bash
cd ~/facy-app

# Исправляем config.py - добавляем ENVIRONMENT
if ! grep -q "ENVIRONMENT: str" config.py; then
    sed -i '/WEBAPP_URL: str/a\    \n    # Environment\n    ENVIRONMENT: str = "production"  # development, production' config.py
fi

# Добавляем extra = "ignore"
if ! grep -q 'extra = "ignore"' config.py; then
    sed -i '/case_sensitive = True/a\        extra = "ignore"  # Игнорировать дополнительные поля из .env' config.py
fi

# Исправляем импорты в api/main.py
sed -i 's/from \.schemas import/from api.schemas import/g' api/main.py
sed -i 's/from \. import payments/from api import payments/g' api/main.py

# Удаляем version из docker-compose.yml
sed -i '/^version:/d' docker-compose.yml

# Исправляем models.py - обновляем Base на DeclarativeBase
sed -i 's/from sqlalchemy.ext.declarative import declarative_base/from sqlalchemy.orm import DeclarativeBase, relationship/g' database/models.py
if grep -q "^Base = declarative_base()" database/models.py; then
    sed -i 's/^Base = declarative_base()$/class Base(DeclarativeBase):\n    """Базовый класс для всех моделей"""\n    pass/g' database/models.py
fi

# КРИТИЧЕСКОЕ: metadata -> transaction_metadata
sed -i 's/^    metadata = Column(Text, nullable=True)/    transaction_metadata = Column(Text, nullable=True)/g' database/models.py

# Обновляем healthcheck в docker-compose.prod.yml
if ! grep -q "start_period" docker-compose.prod.yml; then
    sed -i '/retries: 3$/a\      start_period: 40s' docker-compose.prod.yml 2>/dev/null || sed -i '/retries: 5$/a\      start_period: 40s' docker-compose.prod.yml
    sed -i 's/retries: 3/retries: 5/g' docker-compose.prod.yml
fi

echo "✅ Все файлы исправлены!"
```

### 3.5. Сборка и запуск приложения

```bash
cd ~/facy-app

# Сборка образов
docker compose -f docker-compose.prod.yml build --no-cache

# Запуск приложения
docker compose -f docker-compose.prod.yml up -d

# Проверка статуса
docker compose -f docker-compose.prod.yml ps

# Просмотр логов
docker compose -f docker-compose.prod.yml logs -f
```

**Подождите 30 секунд и проверьте:**

```bash
# Проверка health endpoint
curl http://localhost:8000/health

# Должно вернуться: {"status": "healthy"}
```

---

## 4. Настройка веб-приложения

### 4.1. Настройка DNS для домена onlyface.art

**В панели регистратора домена `onlyface.art`:**

1. Добавьте A-запись:
   ```
   Тип: A
   Имя: @ (или оставьте пустым)
   Значение: 158.160.96.182
   TTL: 3600
   ```

2. Добавьте A-запись для www:
   ```
   Тип: A
   Имя: www
   Значение: 158.160.96.182
   TTL: 3600
   ```

3. Дождитесь распространения DNS (5-30 минут)

4. Проверьте DNS:
   ```bash
   dig onlyface.art
   # или
   nslookup onlyface.art
   ```

### 4.2. Установка и настройка Nginx

**На сервере выполните:**

```bash
cd ~/facy-app

# Установка Nginx
sudo apt update
sudo apt install -y nginx

# Создание конфигурации
sudo tee /etc/nginx/sites-available/onlyface > /dev/null <<'EOF'
server {
    listen 80;
    server_name onlyface.art www.onlyface.art;

    client_max_body_size 100M;

    location / {
        proxy_pass http://localhost:8000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }

    location /static/ {
        alias /home/ubuntu/facy-app/static/;
        expires 30d;
        add_header Cache-Control "public, immutable";
    }
    
    location /uploads/ {
        alias /home/ubuntu/facy-app/uploads/;
        expires 7d;
        add_header Cache-Control "public";
    }
    
    location /generated/ {
        alias /home/ubuntu/facy-app/generated/;
        expires 7d;
        add_header Cache-Control "public";
    }

    location /health {
        proxy_pass http://localhost:8000/health;
        access_log off;
    }
}
EOF

# Активация конфигурации
sudo ln -sf /etc/nginx/sites-available/onlyface /etc/nginx/sites-enabled/
sudo rm -f /etc/nginx/sites-enabled/default

# Проверка конфигурации
sudo nginx -t

# Перезапуск Nginx
sudo systemctl restart nginx
sudo systemctl enable nginx
```

### 4.3. Установка SSL сертификата

```bash
# Установка Certbot
sudo apt install -y certbot python3-certbot-nginx

# Получение SSL сертификата
sudo certbot --nginx -d onlyface.art -d www.onlyface.art --non-interactive --agree-tos --email admin@onlyface.art

# Проверка автоматического обновления
sudo certbot renew --dry-run
```

### 4.4. Обновление WEBAPP_URL

```bash
cd ~/facy-app
sed -i "s|WEBAPP_URL=.*|WEBAPP_URL=https://onlyface.art|g" .env

# Перезапуск приложения
docker compose -f docker-compose.prod.yml restart
```

### 4.5. Настройка Firewall

```bash
# Настройка UFW
sudo ufw allow 22/tcp
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp
sudo ufw --force enable
```

---

## 5. Настройка Telegram Bot

### 5.1. Настройка Menu Button

1. Откройте [@BotFather](https://t.me/BotFather) в Telegram
2. Отправьте `/mybots`
3. Выберите вашего бота
4. Выберите **"Bot Settings"** → **"Menu Button"**
5. Включите **"Menu Button"**
6. В поле **"URL"** введите: `https://onlyface.art`
7. Нажмите **"Save"**

### 5.2. Проверка бота

1. Откройте бота в Telegram
2. Отправьте `/start`
3. Нажмите кнопку **"🚀 Открыть приложение"**
4. Должна открыться Web App на `https://onlyface.art`

---

## 6. Проверка работы

### 6.1. Проверка API

```bash
# Health endpoint
curl https://onlyface.art/health
# Должно вернуться: {"status": "healthy"}

# Проверка в браузере
# Откройте: https://onlyface.art/health
```

### 6.2. Проверка веб-сайта

1. Откройте в браузере: `https://onlyface.art`
2. Должна открыться главная страница приложения

### 6.3. Проверка Telegram Mini App

1. Откройте бота в Telegram
2. Нажмите кнопку **"🚀 Открыть приложение"**
3. Должна открыться Web App

### 6.4. Проверка базы данных

```bash
# Установка PostgreSQL клиента
sudo apt install -y postgresql-client

# Подключение к базе (замените на ваши данные)
psql "host=c-xxxxx.rw.mdb.yandexcloud.net port=6432 dbname=onlyface_db user=onlyface_user password=ВАШ_ПАРОЛЬ sslmode=require"

# Проверка таблиц
\dt

# Проверка данных
SELECT COUNT(*) FROM users;
SELECT COUNT(*) FROM generations;

# Выход
\q
```

---

## 7. Мониторинг и обслуживание

### 7.1. Просмотр логов

```bash
# Все логи
docker compose -f docker-compose.prod.yml logs -f

# Логи API
docker compose -f docker-compose.prod.yml logs -f api

# Логи бота
docker compose -f docker-compose.prod.yml logs -f bot

# Логи Nginx
sudo tail -f /var/log/nginx/access.log
sudo tail -f /var/log/nginx/error.log
```

### 7.2. Статус контейнеров

```bash
docker compose -f docker-compose.prod.yml ps
docker stats
```

### 7.3. Перезапуск приложения

```bash
cd ~/facy-app

# Перезапуск
docker compose -f docker-compose.prod.yml restart

# Полный перезапуск
docker compose -f docker-compose.prod.yml down
docker compose -f docker-compose.prod.yml up -d
```

### 7.4. Обновление приложения

```bash
cd ~/facy-app

# Обновление кода
git pull

# Пересборка и перезапуск
docker compose -f docker-compose.prod.yml build
docker compose -f docker-compose.prod.yml up -d
```

### 7.5. Резервное копирование базы данных

```bash
# Экспорт данных из PostgreSQL
pg_dump "host=c-xxxxx.rw.mdb.yandexcloud.net port=6432 dbname=onlyface_db user=onlyface_user password=ВАШ_ПАРОЛЬ sslmode=require" > backup_$(date +%Y%m%d_%H%M%S).sql

# Импорт данных
psql "host=c-xxxxx.rw.mdb.yandexcloud.net port=6432 dbname=onlyface_db user=onlyface_user password=ВАШ_ПАРОЛЬ sslmode=require" < backup_20240101_120000.sql
```

**Примечание:** Яндекс.Облако автоматически делает резервные копии PostgreSQL.

### 7.6. Мониторинг в Яндекс.Облаке

1. **Мониторинг ВМ:**
   - В консоли: **Compute Cloud** → **Виртуальные машины** → ваша ВМ → **Мониторинг**
   - Просматривайте: CPU, RAM, диск, сеть

2. **Мониторинг PostgreSQL:**
   - В консоли: **Managed Databases** → **PostgreSQL** → ваш кластер → **Мониторинг**
   - Просматривайте: CPU, память, диск, подключения, запросы

---

## 🐛 Решение проблем

### Проблема: Контейнер не запускается

```bash
# Проверьте логи
docker compose -f docker-compose.prod.yml logs api --tail=100

# Проверьте статус
docker compose -f docker-compose.prod.yml ps -a

# Пересоберите образы
docker compose -f docker-compose.prod.yml build --no-cache
```

### Проблема: Не удается подключиться к PostgreSQL

```bash
# Проверьте строку подключения в .env
cat .env | grep DATABASE_URL

# Проверьте доступность хоста
ping c-xxxxx.rw.mdb.yandexcloud.net

# Проверьте группы безопасности в Яндекс.Облаке
# Порт 6432 должен быть открыт для вашей ВМ
```

### Проблема: SSL не устанавливается

```bash
# Проверьте DNS
dig onlyface.art

# Проверьте доступность домена
curl -I http://onlyface.art

# Попробуйте получить сертификат вручную
sudo certbot --nginx -d onlyface.art --debug
```

### Проблема: Nginx возвращает 502 Bad Gateway

```bash
# Проверьте, что API работает
curl http://localhost:8000/health

# Проверьте логи Nginx
sudo tail -f /var/log/nginx/error.log

# Проверьте логи Docker
docker compose -f docker-compose.prod.yml logs api
```

---

## 📊 Архитектура финального решения

```
┌─────────────────────────────────────────────┐
│         Яндекс.Облако                       │
├─────────────────────────────────────────────┤
│                                             │
│  ┌──────────────────────────────────────┐  │
│  │  ВМ (onlyface-server)                │  │
│  │  IP: 158.160.96.182                  │  │
│  ├──────────────────────────────────────┤  │
│  │  ┌────────────┐  ┌──────────────┐   │  │
│  │  │  Nginx     │  │  Docker API  │   │  │
│  │  │  :80, :443 │  │  :8000       │   │  │
│  │  └────────────┘  └──────────────┘   │  │
│  │       │                │             │  │
│  │       └────────┬───────┘             │  │
│  │                │                     │  │
│  │       ┌────────▼────────┐            │  │
│  │       │  Docker Bot     │            │  │
│  │       └─────────────────┘            │  │
│  └──────────────────────────────────────┘  │
│                                             │
│  ┌──────────────────────────────────────┐  │
│  │  Managed PostgreSQL                   │  │
│  │  Кластер: onlyface-postgresql         │  │
│  │  БД: onlyface_db                      │  │
│  └──────────────────────────────────────┘  │
│                                             │
└─────────────────────────────────────────────┘
         │
         │ HTTPS
         ▼
┌─────────────────────────────────────────────┐
│  Интернет                                    │
│  Домен: onlyface.art                        │
│  Telegram Mini App                           │
└─────────────────────────────────────────────┘
```

---

## ✅ Чеклист развертывания

- [ ] Создана ВМ в Яндекс.Облаке
- [ ] Настроены группы безопасности
- [ ] Создан кластер PostgreSQL
- [ ] Настроен DNS для onlyface.art
- [ ] Установлен Docker и Docker Compose
- [ ] Клонирован репозиторий
- [ ] Настроен файл .env
- [ ] Исправлены все проблемы в коде
- [ ] Приложение собрано и запущено
- [ ] Установлен и настроен Nginx
- [ ] Установлен SSL сертификат
- [ ] Настроен Menu Button в BotFather
- [ ] Проверена работа API
- [ ] Проверена работа веб-сайта
- [ ] Проверена работа Telegram Mini App
- [ ] Настроен мониторинг

---

## 🎉 Готово!

Ваше приложение **OnlyFace.art** полностью развернуто и готово к работе!

**Доступно:**
- 🌐 Веб-сайт: https://onlyface.art
- 🤖 Telegram Bot: через @BotFather
- 📱 Telegram Mini App: через кнопку в боте
- 🗄️ База данных: PostgreSQL в Яндекс.Облаке
- 🔒 SSL: автоматически обновляется

**Полезные команды:**

```bash
# Просмотр логов
docker compose -f docker-compose.prod.yml logs -f

# Перезапуск
docker compose -f docker-compose.prod.yml restart

# Статус
docker compose -f docker-compose.prod.yml ps

# Обновление
cd ~/facy-app && git pull && docker compose -f docker-compose.prod.yml up -d --build
```

---

## 📚 Дополнительные ресурсы

- **Подробная инструкция по PostgreSQL:** [POSTGRESQL_SETUP.md](POSTGRESQL_SETUP.md)
- **Инструкция по веб-развертыванию:** [WEB_DEPLOY_CIS.md](WEB_DEPLOY_CIS.md)
- **Документация Яндекс.Облака:** https://cloud.yandex.ru/docs

