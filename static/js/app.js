...
function initHeaderButtons() {
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', () => {
            showNotification('Функция входа будет доступна в следующей версии', 'info');
        });
    }
...
function initButtons() {
    ...
    if (getLinkBtn) {
        getLinkBtn.addEventListener('click', handleGetPartnerLink);
    }
...
async function handleGetPartnerLink() {
    ...
    // Здесь должна быть логика получения партнерской ссылки
    // Пока просто показываем уведомление
    const refLink = `https://t.me/your_bot?start=ref_${userData?.telegram_id || 'XXX'}`;
    
    // Копируем в буфер обмена
    if (navigator.clipboard) {
        await navigator.clipboard.writeText(refLink);
        showNotification('Партнерская ссылка скопирована в буфер обмена!', 'success');
    } else {
        showNotification(`Ваша партнерская ссылка: ${refLink}`, 'info');
    }
}
